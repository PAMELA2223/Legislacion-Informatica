...
import { CrearTareaUseCase } from "@/modules/tutoring/application/tutoring-plan.use-cases";
...
export async function POST(
  ...
  try {
    ...
    const useCase = new CrearTareaUseCase(repo);
    await useCase.execute(assignmentId, body);
    if (auth.docenteId) await registrarLog(prisma, auth.docenteId, "CREAR_TAREA", "tutoria_tarea", assignmentId);
