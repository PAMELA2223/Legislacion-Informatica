...
import { ListarInfografiasAdminUseCase } from "@/modules/admin/application/admin.use-cases";
...
export default async function AdminInfografiasPage() {
  ...
  const infografias = await new ListarInfografiasAdminUseCase(repo).execute();
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {infografias.map((i) => (
          <div key={i.id} className="rounded-xl border border-border bg-surface p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {i.categoria} · {i.fuente}
              </p>
            </div>
            <div className="flex items-center gap-1">
              <Link
                href={`/admin/infografias/${i.id}`}
                className="rounded-lg p-2 text-muted-foreground hover:bg-background-secondary"
              >
                <Pencil className="w-4 h-4" />
              </Link>
              <DeleteButton
                url={`/api/admin/infografias/${i.id}`}
                confirmMessage={`¿Eliminar la infografía "${i.titulo}"?`}
              />
            </div>
          </div>
        ))}
        {infografias.length === 0 && (
          <p className="text-sm text-muted-foreground">Todavía no hay infografías.</p>
        )}
      </div>
    </div>
  );
