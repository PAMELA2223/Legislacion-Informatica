...
| **Estudiante** | Rol por defecto al registrarse. Accede a módulos, biblioteca, evaluaciones, casos prácticos, autoevaluación, foro, ranking, retos, glosario y noticias. |
| **Administrador** | Acceso al panel `/admin`: gestiona usuarios, contenido (cursos, biblioteca, casos prácticos, evaluaciones, glosario, noticias), foro, estadísticas y logs de auditoría. |
...
Administrador, contacta a un administrador de la plataforma.
...
|---|---|
| **Usuarios** (`/admin/usuarios`) | Ver usuarios registrados y cambiar su rol (Estudiante / Administrador). |
...
navegación de estudio (`/modulos`, `/biblioteca`, etc.) es específica del
rol Estudiante, ya que un administrador no "cursa" la plataforma, la
gestiona.
